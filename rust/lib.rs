...
mod _lib {
    ...
    fn solve(size: usize, zeros: Vec<Bound<'_, PyInt>>) -> PyResult<(BigInt, Vec<BigInt>)> {
        let zeros: Vec<BigInt> = zeros.iter().map(|num| num.extract().unwrap()).collect();
        // gauss elimination
        let mut done = 0;
        // for each bit, record its row with the bit set to 1
        let mut mapping = vec![BigInt::ZERO; size];
        ...
        for i in 0..size {
            ...
            base_res.set_bit(i as u64 + 1, mapping[i].bit(0));
        }
        ...
        for bit in free {
            ...
            for i in 0..size {
                if mapping[i].bit((bit + 1) as u64) || i == bit {
                    c.set_bit((i + 1) as u64, true);
                }
            }
            contributions.push(c);
        }
        Ok((base_res, contributions))
    }
}
